...
STATES=(1 2 4 5 6 8 9 10 12 13 {15..42} {44..51} {53..56})
################################################################################
...
do 
    ...
    do 
        ...
        if [[ $geography == 'state' ]]
        then
            call=$endpoint'get='$variables'&for='$geography':'$state
        else
            call=$endpoint'get='$variables'&for='$geography':*&in=state:'$state
        fi
        # echo $call
